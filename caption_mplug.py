...
def main(args, config):
    ...
    print('args.save_HP_labels: ', args.save_HP_labels)
    ...
    train_loader, val_loader, test_loader = create_loader(datasets,samplers,
                                              batch_size=[config['batch_size_train'],config['batch_size_test'], 
                                                          config['batch_size_test']], num_workers=[8,8,8], 
    ...
    model = MPLUG(config=config, tokenizer=tokenizer, use_PR = args.use_PR)
...
#     dist.barrier()
    for epoch in range(start_epoch, max_epoch):
        print('#'*77)
        print(f'Epoch {epoch}, best results so far:')
        for key in best_results.keys():
            print(f'{key}: {best_results[key]} (epoch: {best_results_epoch[key]})')
        print('#'*77)
        if epoch > 0:
        ...
        if epoch >= args.eval_start_epoch:
            ...
            result_file = save_result(vqa_result, args.save_results_dir, f'Val_results_seed_{args.seed}_epoch_{epoch}')
            ...
            if utils.is_main_process():
                ...
                for key in best_results.keys():
                    if result[key] > best_results[key]:
                        best_results[key] = result[key]
                        best_results_epoch[key] = epoch
        
        print('save_HP_labels: ', args.save_HP_labels)
        if args.save_HP_labels and epoch >= args.eval_start_epoch:
            vqa_result_train = evaluation(model, train_loader, tokenizer, device, config) 
            result_file_train = save_result(vqa_result_train, args.save_results_dir, 
                                            f'Train_results_seed_{args.seed}_epoch_{epoch}')
            print('Saved HP label source')
...
#             with open(os.path.join(args.output_dir, "metric_log.txt"), "a") as f:
#                 f.write(json.dumps(result) + "\n")
                
#             with open(os.path.join(args.output_dir, "metric_log.json"), "a") as f:
#                 json.dump(result, f)
...
if __name__ == '__main__':
    ...
    #### CONmPLUG-related
    parser.add_argument('--save_HP_labels', default = False, type=boolean_string)
