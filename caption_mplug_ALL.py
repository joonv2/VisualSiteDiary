...
def main(args, config):
    ...
    train_loader, val_loader_ACID_both, val_loader_SAFE_both, val_loader_ACTV_both, val_loader_all \
    = create_loader(datasets,samplers, batch_size=[config['batch_size_train'],config['batch_size_test'], 
                                                   config['batch_size_test'], config['batch_size_test'], 
                                                   config['batch_size_test']],
                    num_workers=[8,8,8,8,8], 
                    is_trains=[True, False, False, False, False], 
                    collate_fns=[coco_collate_fn, coco_collate_fn, coco_collate_fn, coco_collate_fn, coco_collate_fn]) 

    val_loaders = [val_loader_ACID_both, val_loader_SAFE_both, val_loader_ACTV_both, val_loader_all]
...
#                 torch.save({
#                     'model': model_without_ddp.state_dict()
#                     }, os.path.join('./', f'mPLUG_PC_seed5_ep{epoch}.pth'))
            print('#'*77)
            print(f'Epoch {epoch}, ACID both, best results so far:')
            for key in best_results[0].keys():
                print(f'{key}: {best_results[0][key]: .4f} (epoch: {best_results_epoch[0][key]})')
            print('#'*77)
            print(f'Epoch {epoch}, SAFE both, best results so far:')
            for key in best_results[1].keys():
                print(f'{key}: {best_results[1][key]: .4f} (epoch: {best_results_epoch[1][key]})')
            print('#'*77)
            print(f'Epoch {epoch}, ACTV both, best results so far:')
            for key in best_results[2].keys():
                print(f'{key}: {best_results[2][key]: .4f} (epoch: {best_results_epoch[2][key]})')
            print('#'*77)
            print(f'Epoch {epoch}, All datasets, best results so far:')
            for key in best_results[3].keys():
                print(f'{key}: {best_results[3][key]: .4f} (epoch: {best_results_epoch[3][key]})')
            print('#'*77)
