...
def create_dataset(dataset, config, epoch=None):
    ...
    if dataset=='pretrain':
        ...
        val_dataset = coco_dataset(config['val_file'], test_transform, config['coco_root_val'], max_words=config['max_length'],
                                   read_local_data=config['read_local_data'], is_train=False, add_object=config['add_object'])
        
        test_dataset = coco_dataset(config['test_file'], test_transform, config['coco_root_test'], max_words=config['max_length'],
                                    read_local_data=config['read_local_data'], is_train=False, add_object=config['add_object'])
...
def create_dataset_all(dataset, config, epoch=None):
    normalize = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))
...
def create_dataset_double(dataset, config, epoch=None):
    normalize = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))
